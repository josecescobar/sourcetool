...
export class BillingService {
  ...
  async createCheckoutSession(teamId: string, planTier: PlanTier) {
    ...
    const session = await this.stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.WEB_URL || 'http://localhost:3000'}/settings?tab=billing&success=true`,
      cancel_url: `${process.env.WEB_URL || 'http://localhost:3000'}/settings?tab=billing&canceled=true`,
  ...
  async createPortalSession(teamId: string) {
    ...
    const session = await this.stripe.billingPortal.sessions.create({
      ...
      return_url: `${process.env.WEB_URL || 'http://localhost:3000'}/settings?tab=billing`,
    });
