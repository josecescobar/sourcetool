// Amazon standard-size cutoff: over any of these thresholds counts as oversize.
export function isOversizeDimensions(
  dimensions: { lengthInches: number; widthInches: number; heightInches: number; weightPounds: number },
): boolean {
  return dimensions.lengthInches > 18 || dimensions.widthInches > 14 ||
...
export function getAmazonStorageFee(
  ...
  const isOversize = isOversizeDimensions(dimensions);
